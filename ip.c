...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (len < hlen)
    {
        errorf("header length too short", hlen, len);
        return;
    }
    total = ntoh16(hdr->total);
    if (len < total)
    {
        errorf("total length too short", total, len);
        return;
    }
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    { /* NOTE: I'll rewrite this block later. */
        ...
        if (dst != iface->broadcast && dst != IP_ADDR_BROADCAST && (dst & iface->netmask) != (iface->unicast & iface->netmask))
        {
            errorf("destination unreachable for dst=%s", ip_addr_ntop(dst, addr, sizeof(addr)));
            return -1;
        }
        id = ip_generate_id();
